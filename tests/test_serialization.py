...
def test_processing_on_grpc_server():
    ...
    from ai_common_utils.files import convert_from
    from ai_grpc_utils.generators import grpc_runner, connect_to_grpc_server, serve
    from ai_grpc_utils.encoders import BytesEncoder

    PACKAGE_NAME = "ai_gu_serialization"

    class RealizationSer:
        ...
        @grpc_runner(encoder=BytesEncoder)
        def process(self, arg: bytes):
            arg = convert_from(arg, ["json", "base64"])
            assert arg == b"Some text"
            arg = arg.decode()
            return f"Processed! ({arg=})"

    def serving():
        client = RealizationSer()
    ...
    def bytes_processing_on_grpc_server():
        time.sleep(1)
        proc = run_server()
        ...
        res = esa.process(b"Some text")

        shutdown_server(proc)

        assert res == "Processed! (arg='Some text')"

    bytes_processing_on_grpc_server()


if __name__ == "__main__":
    test_processing_on_grpc_server()
